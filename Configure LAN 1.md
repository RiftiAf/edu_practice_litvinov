...
## Вывод
В третьей части работы настроен MLS. Включена IP-маршрутизация, созданы VLAN 100 и 200 с именами и с соответствующими SVI для связи между ними. Интерфейсы f0/1-3 переведны в режим маршрутизируемых портов с назначением IP-адресов для подключения к маршрутизаторам. Првоерка пингом подтвердила корректность настроек.
...
4. Проверить связность и удаленый доступ.
...
На маршрутизаторах R2 и R3 настроил ACL, который позволяет маршрутизаторам пинговать любые устройства, но запрещает отвечать на входящте ping-запросы.
<p align="center">
...
3. На R3 и R2 настроен ACL 101, запрешающий отправку ICMP-овтетов, что позволяет маршрутизаторам пинговать другие устройства, но не отвечать на внешние запросы.
